...
export default function KpiSection({
  ...
  // Las barras de progreso arrancan en 0 y animan a su valor real tras el
  // montaje — sin esto, `transition-all duration-1000` nunca se dispara
  // porque el ancho llega ya calculado en el primer render.
          ...
          {overBudget ? (
            ...
            <div className="flex items-center gap-2 mt-2.5">
              <div className="flex-1 bg-slate-100 rounded-full h-2 overflow-hidden">
                <div className="bg-gradient-to-r from-sky-400 to-sky-600 h-2 rounded-full transition-all duration-1000 ease-out" style={{ width: barsFilled ? `${barWidth}%` : "0%" }} />
              </div>
      ...
      <motion.div variants={itemVariants}>
        <KpiCard icon={<Layers className="h-5 w-5" />} label="Estado de Proyectos" accent="text-sky-600" borderAccent="border-l-sky-400" onInspect={() => setInspecting("projects")}>
          ...
          <div className="flex w-full h-1.5 rounded-full overflow-hidden bg-slate-100 mt-2.5">
            <div className="bg-sky-400 h-full transition-all duration-1000 ease-out" style={{ width: barsFilled ? `${activePct}%` : "0%" }} />
            <div className="bg-emerald-500 h-full transition-all duration-1000 ease-out" style={{ width: barsFilled ? `${completedPct}%` : "0%" }} />
          </div>
