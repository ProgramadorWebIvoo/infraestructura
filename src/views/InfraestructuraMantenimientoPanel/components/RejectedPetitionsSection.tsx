...
import { AlertTriangle, Pencil, User, XCircle } from "lucide-react";
...
export default function RejectedPetitionsSection({
  ...
  const rejected = projects.filter((p) => p.status === ProjectStatus.RECHAZADO_CIERRE);

  if (rejected.length === 0) return null;

  return (
    <motion.div variants={bannerVariants} initial="hidden" animate="visible">
      <Card accent="danger" hoverable={false}>
        <SectionHeader
        ...
        <div className="mt-4 space-y-3">
          <AnimatePresence>
            {rejected.map((p) => {
              const log = latestRejectionLog(auditLogs, p.id);
              return (
                <motion.div
                  key={p.id}
                  ...
                  animate="visible"
                  exit="exit"
                  className="p-4 rounded-xl border border-danger-100 bg-danger-50/40"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="shrink-0 inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold text-danger-700 bg-white border border-danger-200 hover:bg-danger-50 transition-colors cursor-pointer"
                    >
