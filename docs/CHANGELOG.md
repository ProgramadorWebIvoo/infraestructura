...
- Qué:
  - **Constantes como fuente única**: `src/constants/aiProviders.ts` (AI_PROVIDERS, PROVIDER_LABELS, PROVIDER_COLORS con nueva clave `bar`, `providerColor()`) y `src/constants/aiModels.ts` (catálogo `PROVIDER_MODELS` por proveedor, editable en código). El hook `useAIConfig` re-exporta las constantes (sin romper imports) y `loadProviderModels` hace merge `{...PROVIDER_MODELS, ...data}`: el endpoint solo puede **extender** el catálogo, nunca vaciarlo; el estado inicial ya no es `[]` sino el catálogo base (sin flash de selector vacío).
  - **Lógica pura extraída**: `src/utils/aiFormat.ts` (`formatAiCost`: `< $0.01` para null/NaN/negativos/<1 centavo) y `src/views/AIConfigPanel/aiConfigForm.ts` (`validateConfigForm`, `buildUpdatePayload`, `DEFAULT_MAX_TOKENS=4096`) — `index.tsx` ya no castea `Record<string,unknown>` ni valida inline.
  - **Componentes descompuestos**: `AIConfigTable` (RowActionButton + guard runtime `maskApiKey` que enmascara por completo claves >8 chars — hallazgo ALTO de la auditoría), `UsageDashboard` (UsageKpis + ProviderUsageBar con share del total), `AIConfigFormModal` (AiFormField + TogglePill + **labels asociados a controles vía `htmlFor`/`id`** — fix a11y real), `ProviderIcon` (lee de constantes), `MiniBarChart` movido de `components/UI/` a `AIConfigPanel/` (único consumidor es `UsageDashboard`; el bug de "Rendered fewer hooks" al pasar de datos vacíos→no vacíos se corrigió moviendo el `useMemo` tras el early return).
  - **Backend alineado a docs oficiales**: `OpenAIProvider` y `testOpenAI` usan `max_completion_tokens` (deprecado/rechazado `max_tokens` en gpt-5/o-series); `AnthropicProvider` ya no envía `temperature` (no soportada en Opus 4.x/Sonnet 4.5+/Haiku 4.5 → 400) y mantiene headers `x-api-key` + `anthropic-version`; `GeminiProvider` ya usaba `x-goog-api-key` + `generationConfig.maxOutputTokens` (correcto, se deja igual). `AVAILABLE_MODELS` del controller sincronizado con `aiModels.ts` (faltaban `gpt-5.4-mini`, `gpt-5.2`, `o3`, `o4-mini`, `claude-opus-5`, `claude-sonnet-4-6`, `gemini-2.0-flash`).
  - **Testing**: 10 archivos nuevos en frontend (unit + integración con hook mockeado) y `tests/Feature/AiProviderPayloadTest.php` en backend con `Http::assertSent` verificando `max_completion_tokens` presente/`max_tokens` ausente (OpenAI), `temperature` ausente + headers (Anthropic) y `generationConfig.maxOutputTokens` + header key (Gemini).
- Por qué / causa raíz: el usuario pidió mejorar la vista metódica y técnicamente bajo SOLID, con testing y sin romper nada, consultando la documentación de los 3 proveedores; la vista acumulaba lógica inline en componentes, constantes duplicadas, y los payloads de OpenAI/Anthropic usaban campos no vigentes.
- Archivos: `src/constants/aiProviders.ts` [N], `src/constants/aiModels.ts` [N], `src/utils/aiFormat.ts` [N], `src/views/AIConfigPanel/aiConfigForm.ts` [N], `src/views/AIConfigPanel/{index,AIConfigTable,UsageDashboard,ProviderIcon,MiniBarChart}.tsx`, `src/components/Modals/AIConfigFormModal.tsx`, `src/hooks/useAIConfig.ts`, `docs/DOCUMENTACION_PROYECTO.md`, `src/__tests__/**` (+10 archivos), `src/__tests__/hooks/useAIConfig.test.ts`; backend: `app/Services/AI/Providers/{OpenAIProvider,AnthropicProvider}.php`, `app/Http/Controllers/Api/AiConfigController.php`, `tests/Feature/AiProviderPayloadTest.php` [N].
- Verificación: `tsc --noEmit` 0 errores; frontend **538/538** (antes 479); `vite build` OK; backend suite IA **27/27** (incl. 3 nuevos payload tests).
...
- Tipo: feature + refactor + fix
- Qué:
  - **Animación de colapso profesional**: las etiquetas de navegación ya no aparecen/desaparecen con `display:none` (salto seco) ni con el hack `absolute pl-10` que sacaba el texto del flujo y rompía el layout expandido. Ahora `sidebarTextClass` desvanece el label (`transition-opacity`) con `max-width` snap, y el `aside` anima `width` en 300ms con easing Material `cubic-bezier(.4,0,.2,1)`. El chevron del toggle rota con spring de framer-motion. **Fix de artefacto visual (línea negra momentánea al colapsar)**: causado por (a) la costura de subpíxel del borde oscuro del sidebar contra el contenido claro durante la animación de `width` — resuelto promoviendo el `aside` a su propia capa con `transform-gpu` (`translateZ(0)`); y (b) el borde de recorte del texto durante la animación de `max-width` — resuelto animando solo `opacity` y aplicando el ancho instantáneamente. `transition-[width,transform,translate]` (incluye `translate` porque en Tailwind v4 el drawer móvil usa la propiedad CSS `translate`, no `transform`).
  - **Centrado real de iconos colapsados**: `navLinkClass` pasa de `px-3` persistente + `gap-0` a `justify-center gap-0 px-0` cuando colapsa — el icono queda centrado en `w-16` (antes quedaba a 12px del borde izquierdo). Aplica a los 7 links, botón Configuración, logout y avatar del footer. El botón de colapso tenía `aria-label` y rotación invertidos (expandido apuntaba a expandir y viceversa) — corregidos.
  - **Render boundaries**: `SidebarNav` envuelto en `memo` (único — había doble `memo`), `onClose`/`onToggleCollapse` estabilizados con `useCallback` en el layout (hoisted fuera del JSX), y `canAccess` memoizado con `useCallback` en `useRoleAccess` — antes se recreaba en cada render de App, anulando el memo. Resultado: el sidebar ya no re-renderiza al navegar entre rutas. El dropdown de Configuración se extrajo a `ConfigDropdown.tsx` (memo, estado `isConfigOpen` aislado) — abrir/cerrar el submenú ya no re-renderiza los 7 NavLink principales.
  - **Persistencia del colapso**: el estado colapsado sobrevive al refresh vía `localStorage` (`ivoo.sidebar.collapsed`), con inicialización lazy y `useEffect` de escritura.
  - **Fixes de higiene**: `SidebartextClass` invocado sin `()` en el tagline del header (rendía el source de la función como className); import de `framer-motion` (dependencia transitiva de `motion`) → `motion/react` (dependencia directa, consistente con el layout); `ChevronLeft` sin usar; tipo `Boolean` → `boolean`; stray `rotate` en el chevron; factories de clases extraídas a `sidebarNavClasses.ts` compartido entre `SidebarNav` y `ConfigDropdown`.
- Por qué / causa raíz: el colapso funcionaba pero con saltos visuales (labels `hidden`), iconos descentrados (padding residual), y re-renders innecesarios del sidebar en cada cambio de ruta (callbacks/factories con identidad nueva por render + `canAccess` sin memoizar).
- Archivos: `src/components/UI/SidebarNav.tsx`, `src/components/UI/ConfigDropdown.tsx` [NUEVO], `src/components/UI/sidebarNavClasses.ts` [NUEVO], `src/components/Layout/AuthenticatedLayout.tsx`, `src/hooks/useRouting.ts`, `src/__tests__/components/UI/SidebarNav.test.tsx`, `src/__tests__/components/Layout/AuthenticatedLayout.test.tsx`
- Verificación: `tsc --noEmit` 0 errores; `vite build` OK (CSS genera `max-w-0`/`max-w-40`, `transition-property:width,transform`, easing `cubic-bezier(.4,0,.2,1)`); suite completa — 438/449 tests pasando. Los 11 fallos restantes son pre-existentes del WIP en `useRateLimit.ts`/`LoginScreen` (ajenos a este cambio).

## [2026-07-28] — Fixes de UX/negocio: doble confirmación en Fin de Obra, anticipo de proveedores, tiempo estimado fantasma, botón "X" inerte en SelectModal
- Tipo: fix + feature
- Qué:
  - **Doble confirmación sin contexto en Auditoría de Fin de Obra**: `CompletionAuditSection.tsx` mostraba el mismo botón ("Certificar Calidad y Autorizar Pago") y el mismo modal de confirmación tanto en `EN_EJECUCION` como en `VERIFICANDO_FINALIZACION`, aunque cada clic dispara una acción distinta (`report-finished` vs `verify-completion`). Ahora el badge indica "Paso 1 de 2" / "Paso 2 de 2", hay un texto explicativo por tarjeta, y el botón/modal cambian su copy según el paso ("Reportar Obra como Finalizada" vs "Certificar Calidad y Autorizar Pago").
  - **Anticipo ausente en la propuesta pública de proveedores**: el formulario público (`PropuestaMaterialesPublica/`) no tenía forma de que el proveedor indicara el anticipo requerido; al importar la propuesta hacia `project_proposals` (backend) se hardcodeaba `negotiated_advance_percent = 30` sin importar lo que el proveedor necesitara. Se agregó el campo "Anticipo requerido (%)" en `ProposalDetailsSection.tsx`, se propaga en `index.tsx` y se muestra en el panel interno `SupplierProposalsList.tsx`.
  - **Tiempo estimado "fantasma"**: si el proveedor dejaba el tiempo de ejecución vacío, el backend inventaba 1/2/4 semanas de plazo (ver detalle en el changelog del backend) para poder alimentar la tabla comparativa y la evaluación IA. El frontend ahora muestra "Sin dato" (en vez de "0 semanas") en `ComparativeTableSection.tsx`, `BidEvaluationSection.tsx` y `EvaluacionInteligenteModal` (tabla, rango min–max, tarjeta de ganador y banner de adjudicación) cuando `deliveryWeeks === 0`, y el cálculo del rango min/max excluye esos casos para no arrastrar el mínimo a 0 artificialmente.
  - **Botón "X" inerte en `SelectModal`**: el botón de deselección se renderizaba siempre que había una opción seleccionada y `allowDeselect` (default `true`), sin comprobar si el consumidor pasó `onDeselect` — ninguno de los 4 usos existentes lo pasaba, así que la X aparecía en toda la app sin hacer nada. Ahora el botón solo se renderiza si `onDeselect` está definido. Se cableó `onDeselect` real en los dos selectores opcionales de `BidRegistrationSection.tsx` (obra, proveedor/contratista); los otros 3 usos (`ContractorFormModal`, `MaterialFormModal`, `MaterialAdderSection`), que seleccionan un valor obligatorio, pasan `allowDeselect={false}` explícito en vez de exponer una X sin función.
- Por qué: los primeros tres son hallazgos reportados directamente por el usuario durante pruebas manuales de los flujos de Cierre de Obra y del portal público de proveedores; el cuarto (SelectModal) fue detectado por el usuario al notar que la X nunca limpiaba la selección en ningún panel.
- Archivos: `src/views/CierreObraPanel/CompletionAuditSection.tsx`; `src/views/PropuestaMaterialesPublica/{index,ProposalDetailsSection,types}.tsx`; `src/views/ProveedoresRegistrados/SupplierProposalsList.tsx`; `src/views/AnalistasPanel/{ComparativeTableSection,BidRegistrationSection}.tsx`; `src/views/ProcuraPanel/BidEvaluationSection.tsx`; `src/components/Modals/EvaluacionInteligenteModal/{index,IdleView,ResultView}.tsx`; `src/components/UI/SelectModal.tsx`; `src/views/ProveedoresConfigPanel/ContractorFormModal.tsx`; `src/views/MaterialConfigPanel/MaterialFormModal.tsx`; `src/views/InfraestructuraMantenimientoPanel/MaterialAdderSection.tsx`; `packages/shared/src/types.ts` (nuevo campo `advancePercent` en `SupplierMaterialProposal`).
- Verificación: `tsc --noEmit` 0 errores; suite completa `npx vitest run` — 439/439 tests pasando (34 archivos).
